...
manydogs_data_transformed <- manydogs_data |>
  ...
  mutate(across(c(cbarq_train_1:cbarq_train_8,cbarq_aggression_1:cbarq_miscellaneous_27), as.numeric))  #change character columns to numeric columns so R understands how to use them
...
manydogs_feature_selection <- manydogs_data_transformed |> 
  mutate(training_score = rowMeans(select(manydogs_data_transformed,   #compute average scores of training
         ...
         os_best = ifelse(nonostensive > ostensive, 0, 1)) |>   #create column that otes if the dog was better at the nonostensive task
  select(-c(date:birthdate,breed:gesture_frequency,training_type:years_owned,origin:twocup,nonostensive:right_choice_nonost))#take out the columns we will not be using for the analysis in this tutorial
